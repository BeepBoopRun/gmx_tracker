...
def main():
    ...
    if args.runs_directory:
        run_dir_path = Path(args.runs_directory).absolute()
    else:
    ...
    for id, configs in enumerate(simulation_pool):
        ...
        if not failed_by_deault:
            config_results = run_simulations(
                simulations, early_stop=args.early_stop, config_id=id
            )
            ...
            if config_results.overall_status == SimulationStatus.Failure:
                for sim in config_results.sim_results:
                    if sim.status == SimulationStatus.Failure:
                        ...
                        if pattern:
                            already_on_list = (
                                filter_simulations(pattern, failing_patterns) == []
                            )
                            if already_on_list:
                                continue
    ...
    for res in results:
        print(
            f"Config ID: {res.config_id} Total performance: {res.total_performance} [ns/day] Total steps done: {res.total_steps}"
        )
